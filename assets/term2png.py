"""Render a captured ANSI terminal transcript as a PNG in the brand's frame.
    ...
    outcry roster > assets/roster.txt
    python assets/term2png.py assets/roster.txt assets/roster.png [max_lines]
...
def page(body: str, title: str) -> str:
    return f"""<html><head>{FONT}<style>
      ...
      .bar{{display:flex;gap:8px;align-items:center;padding:8px 14px;border-bottom:1px solid {LINE};
            color:#7A6A56;font-size:12px;letter-spacing:2px;text-transform:uppercase}}
...
async def main() -> None:
    ...
    async with async_playwright() as p:
        ...
        await pg.set_content(page(ansi_to_html(raw), f"outcry {src.stem}"), wait_until="networkidle")
